...
checkThrief()
graf(kart)
makeNewMap(kart, player, thriefs)


algoritm = DFS1(kart)
algoritm.mainAlgoritm(thriefs)
...
while run:
	...
	if (tm == 10):
		...
		player.x = point[way[now]][1]*50
		player.y = point[way[now]][0]*50
		passed.append(way[now])
		now +=1
		tm = 0
	...
	if keys[pygame.K_SPACE]:
		...
		checkThrief()
		algoritm = DFS1(kart)
		algoritm.newAlgoritm(player)
		algoritm.mainAlgoritm()

	# if keys[pygame.K_z]:
	# 	if al == 2:
	# 		al = 0
	# 		algoritm[al].newAlgoritm(player)
